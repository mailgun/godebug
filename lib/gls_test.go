...
func TestContexts(t *testing.T) {
	mgr1 := newStackTraceManager()
	mgr2 := newStackTraceManager()
...
func growStackBy(m int, fn func()) func() {
	n = m
	f = fn
	return recur
}
...
func BenchmarkGetValue(b *testing.B) {
	id := goid()
	mgr := newStackTraceManager()
	mgr.SetValues(gls.Values{"test_key": "test_val"}, growStackBy(500, func() {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			if goid() != id {
				b.FailNow()
